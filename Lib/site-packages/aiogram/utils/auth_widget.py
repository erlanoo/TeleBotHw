...
@deprecated(
    "`generate_hash` is outdated, please use `check_signature` or `check_integrity`",
    stacklevel=3,
)
def generate_hash(data: dict, token: str) -> str:
    ...
    msg = "\n".join(
        "{}={}".format(k, v) for k, v in sorted_params.items() if k != "hash"
    )
    return hmac.new(
        secret.digest(), msg.encode("utf-8"), digestmod=hashlib.sha256
    ).hexdigest()
